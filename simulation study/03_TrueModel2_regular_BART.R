################################################################################
# 03_TrueModel2_regular_BART.R
#
# PURPOSE:
# Fit Regular BART with sparsity (DART) on data generated from True Model 2.
#
...
#
# OUTPUTS:
# - results/TrueModel2/n{1500,2500}/p{500,2000,3000}/regular_BART/rep_{001-500}.RDS
#
################################################################################
...
cat("\n")
cat("================================================================================\n")
cat("  03_TrueModel2_regular_BART.R: Regular BART with Sparsity (DART)\n")
...
cat("DART: ntrees =", ntrees, ", ndpost =", ndpost, ", nskip =", nskip, ", keepevery =", keepevery, "\n")
...
for (n in sample_sizes) {
  for (p_genes in gene_counts) {
    ...
    tryCatch({
      ...
      covariate_fold_frequency <- if (length(union_covariates) > 0) {
        sapply(union_covariates, function(c) sum(sapply(fold_selected_covariates, function(fc) c %in% fc)))
      } else integer(0)
      ...
      result <- list(
        ...
        # Covariate selection (parallel to gene selection)
        union_covariates = union_covariates,
        n_union_covariates = length(union_covariates),
        covariate_fold_frequency = covariate_fold_frequency,
        fold_selected_covariates = fold_selected_covariates,
        fold_covariate_PIPs = fold_covariate_PIPs,
