################################################################################
# 04_TrueModel2_spBART.R
#
# PURPOSE:
# Fit Semi-parametric Probit BART (spBART) on data generated from True Model 2.
#
...
# spBART is CORRECTLY SPECIFIED for this model.
#
...
#
# OUTPUTS:
# - results/TrueModel2/n{1500,2500}/p{500,2000,3000}/spBART/rep_{001-500}.RDS
#
################################################################################
...
cat("\n")
cat("================================================================================\n")
cat("  04_TrueModel2_spBART.R: Semi-parametric Probit BART\n")
cat("  True Model 2: Additive covariate effects (spBART CORRECTLY SPECIFIED)\n")
...
cat("================================================================================\n")
cat("Replicate ID:", rep_id, "\n")
cat("True Model:", true_model, "(spBART CORRECTLY SPECIFIED)\n")
...
for (n in sample_sizes) {
  for (p_genes in gene_counts) {
    ...
    results_dir <- file.path(script_dir, "results", "TrueModel2",
    ...
    tryCatch({
      ...
      fold_beta_draws <- vector("list", K_folds)
      ...
      for (k in 1:K_folds) {
        ...
        # Fit spBART (no test set needed for variable selection)
        ...
        fold_beta_draws[[k]] <- fit_k$beta_draws
      ...
      if (length(union_genes) > 0) {
        ...
        # Get mu_hat = f_hat + Z'beta_hat (full latent mean)
        ...
        # RMSE of latent function (mu_hat vs f_true)
        final_RMSE <- sqrt(mean((mu_hat - f_true)^2))

        # Update beta estimates from final model
        final_beta_hat <- final_fit$beta_hat
        final_beta_sd <- apply(final_fit$beta_draws, 2, sd)
        final_beta_ci_lower <- apply(final_fit$beta_draws, 2, quantile, 0.025)
        final_beta_ci_upper <- apply(final_fit$beta_draws, 2, quantile, 0.975)
        final_beta_bias <- final_beta_hat - beta_true
        final_beta_mse <- final_beta_bias^2
        final_beta_rmse <- abs(final_beta_bias)  # Per covariate (not aggregated)
        final_beta_ci_width <- final_beta_ci_upper - final_beta_ci_lower
        final_beta_coverage <- (beta_true >= final_beta_ci_lower) & (beta_true <= final_beta_ci_upper)

        # Compute posterior probability of correct sign
        final_beta_sign_prob <- sapply(1:ncol(final_fit$beta_draws), function(j) {
          mean(sign(final_fit$beta_draws[, j]) == sign(beta_true[j]))
        })
        names(final_beta_sign_prob) <- colnames(final_fit$beta_draws)

        cat("  Final AUC:", round(final_AUC, 3), "\n")
        cat("  Final Brier:", round(final_Brier, 4), "\n")
        cat("  Final RMSE:", round(final_RMSE, 4), "\n")
        cat("  Final Beta estimates:", paste(round(final_beta_hat, 3), collapse = ", "), "\n")
        cat("  Beta true:", paste(round(beta_true, 3), collapse = ", "), "\n")
        ...
        final_beta_bias      <- na_vec
        final_beta_mse       <- na_vec
        final_beta_rmse      <- na_vec
        final_beta_ci_width  <- na_vec
        final_beta_coverage  <- setNames(rep(NA, J), colnames(Z_cov))
        final_beta_sign_prob <- na_vec
      }
      ...
      result <- list(
        ...
        # Beta estimation (from final model - all per-covariate vectors)
        beta_hat = final_beta_hat,
        beta_sd = final_beta_sd,
        beta_bias = final_beta_bias,
        beta_mse = final_beta_mse,
        beta_rmse = final_beta_rmse,
        beta_ci_lower = final_beta_ci_lower,
        beta_ci_upper = final_beta_ci_upper,
        beta_ci_width = final_beta_ci_width,
        beta_coverage = final_beta_coverage,
        beta_sign_prob = final_beta_sign_prob,
